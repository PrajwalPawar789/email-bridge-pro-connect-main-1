-- Fix the monitoring function to prevent duplicate campaign triggers
...
BEGIN
  ...
  -- IMPROVED: Exclude campaigns that are already 'sent' or recently completed
  FOR campaign_record IN 
    SELECT DISTINCT c.id, c.name, c.status, c.last_batch_sent_at, c.updated_at
    FROM campaigns c
    INNER JOIN recipients r ON c.id = r.campaign_id
    WHERE c.status IN ('ready', 'sending', 'failed') -- Only process these statuses
      AND c.status != 'sent' -- Exclude completed campaigns
      AND r.status = 'pending' -- Only if there are pending recipients
      AND (c.status = 'ready' OR 
           (c.status = 'sending' AND (c.last_batch_sent_at IS NULL OR c.last_batch_sent_at < current_time - INTERVAL '10 minutes')) OR
           (c.status = 'failed'))
      AND c.updated_at < current_time - INTERVAL '2 minutes' -- Don't process recently updated campaigns
  LOOP
    ...
    IF pending_count > 0 THEN
      ...
      IF campaign_record.status != 'sending' OR 
        ...
        WHERE id = campaign_record.id
          AND status != 'sent'; -- Extra safety check
        
        -- Only trigger if we actually updated the campaign
        IF FOUND THEN
