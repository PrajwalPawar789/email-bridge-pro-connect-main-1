-- Fix cron job failures caused by timestamp comparisons and header JSON casting
...
BEGIN
  ...
  ELSE
    RAISE WARNING 'Service role key not configured; skipping HTTP triggers in trigger_next_batch.';
  END IF;
  ...
  LOOP
    ...
    IF pending_count > 0 THEN
      ...
      ELSE
        RAISE WARNING 'Skipped triggering batch for campaign % due to missing service role key',
          campaign_record.id;
      END IF;
    END IF;
  END LOOP;
