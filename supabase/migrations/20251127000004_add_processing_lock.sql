-- Function to clean up stuck processing recipients
CREATE OR REPLACE FUNCTION public.cleanup_stuck_processing_recipients()
RETURNS void
LANGUAGE plpgsql
AS $function$
BEGIN
  -- Reset stuck recipients (processing for > 15 mins)
  -- Case 1: Never sent an email (last_email_sent_at is null) -> pending
  UPDATE recipients
  SET status = 'pending',
      updated_at = NOW()
  ...
  -- Case 2: Have sent an email before -> sent (ready for next step retry)
  UPDATE recipients
  SET status = 'sent',
      updated_at = NOW()
  WHERE status = 'processing'
  ...
  RAISE NOTICE 'Cleaned up stuck processing recipients';
END;
$function$;

-- Update the monitor function to include cleanup
...
BEGIN
  ...
  PERFORM public.cleanup_stuck_processing_recipients(); -- Added cleanup
