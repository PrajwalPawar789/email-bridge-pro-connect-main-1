...
const levelFromEventType = (
  eventType: string,
): WorkflowRuntimeEvent["level"] => {
  const lowered = eventType.toLowerCase();
  if (lowered.includes("fail") || lowered.includes("error")) return "error";
  if (lowered.includes("warning") || lowered.includes("blocked"))
    return "warning";
  return "info";
};
...
const Automations = () => {
  ...
  const [templatePreviewId, setTemplatePreviewId] = useState<string | null>(
    null,
  );
  ...
  const [builderPayload, setBuilderPayload] = useState<BuilderPayload | null>(
    null,
  );
  ...
  const [workspaceView, setWorkspaceView] = useState<"builder" | "templates">(
    "builder",
  );
  const [editorPanel, setEditorPanel] = useState<"essentials" | "advanced">(
    "essentials",
  );
  ...
  const requiresAutomationApproval = Boolean(
    workspace?.requiresApproval.automation,
  );
  const supportsApiWebhooks = workspace
    ? workspace.planFeatures?.apiWebhooks !== false
    : true;
  ...
  const selectedTemplate = useMemo(
    () =>
      (supportsApiWebhooks
        ? templates
        : templates.filter(
            (template) => template.trigger_type !== "custom_event",
          )
  ...
  const editorTriggerSegmentId = String(
    editorTriggerFilters.segment_id || "",
  ).trim();
  const availableTriggerSegments = useMemo(() => {
    if (!editor) return dependencies.contactSegments;
    if (editor.trigger_type !== "list_joined")
    ...
    return dependencies.contactSegments.filter(
      (segment) =>
        ...
        segment.source_list_id === editor.trigger_list_id,
    );
  }, [dependencies.contactSegments, editor]);
  const webhookSecret = String(
    editorTriggerFilters.webhook_secret || "",
  ).trim();
  ...
  const setTriggerType = useCallback(
    (value: AutomationWorkflow["trigger_type"]) => {
      if (!supportsApiWebhooks && value === "custom_event") {
        toast({
          title: "Webhook triggers require an upgrade",
          description:
            "Upgrade to Growth or higher to unlock API and webhook triggers.",
          variant: "destructive",
        });
        return;
      }
      ...
      setEditor((prev) =>
        prev
          ? {
              ...
              trigger_type: value,
              trigger_list_id:
                value === "list_joined" ? prev.trigger_list_id : null,
              trigger_filters: (() => {
                ...
                if (value === "custom_event") {
                  nextFilters.webhook_secret =
                    String(nextFilters.webhook_secret || "").trim() ||
  ...
  useEffect(() => {
    ...
    if (
      !supportedTemplates.some((template) => template.id === templatePreviewId)
    ) {
  ...
  const loadData = useCallback(
    async ({ initial = false }: { initial?: boolean } = {}) => {
      ...
      try {
        const [workflowRows, dependencyRows, templateRows, snapshot] =
          await Promise.all([
        ...
        const activeWorkflowId = selectedWorkflowIdRef.current;
        const resolvedId =
          activeWorkflowId &&
          workflowRows.some((workflow) => workflow.id === activeWorkflowId)
            ? activeWorkflowId
            : nextId;
  ...
  const automationApprovalPending =
    selectedApprovalStatus === "pending_approval";
    ...
    async (
      ...
      if (!supportsApiWebhooks && editor.trigger_type === "custom_event") {
        toast({
          title: "Webhook triggers require an upgrade",
          description:
            "Custom event automations are available on Growth plan or higher.",
          variant: "destructive",
        });
        return null;
      }
  ...
  const submitWorkflowForApproval = useCallback(async () => {
    ...
    if (!payload.checklistPass) {
      toast({
        ...
        description:
          "Fix the publish checklist before submitting for approval.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description:
        "Automation activation is now waiting in the approval queue.",
    });
  ...
  const testWebhookConnection = useCallback(async () => {
    ...
    if (!supportsApiWebhooks) {
      const message =
        "Webhook triggers are available on Growth plan or higher.";
      setWebhookTestState({
        status: "error",
        message,
    ...
    if (!webhookEndpoint) {
      const message =
        "Webhook endpoint is unavailable. Set VITE_SUPABASE_URL and save workflow first.";
      setWebhookTestState({
        status: "error",
        message,
        testedAt: new Date().toISOString(),
    ...
    try {
      ...
      const response = await fetch(webhookEndpoint, {
        ...
        headers: {
          ...
          ...(webhookSecret
            ? { "x-vintro-webhook-secret": webhookSecret }
            : {}),
        },
        body: JSON.stringify({
          event: webhookEventName || "contact_created",
      ...
      if (!response.ok) {
        throw new Error(
          String(
            payload.error || `Webhook endpoint returned ${response.status}.`,
          ),
        );
      }
    ...
    action("apply-template", async () => {
      ...
      if (!supportsApiWebhooks && template.trigger_type === "custom_event") {
        toast({
          ...
          description:
            "Upgrade to Growth or higher to use API and webhook automations.",
          variant: "destructive",
        });
        return;
      }

      const created = await createAutomationWorkflowFromTemplate(
        user.id,
        template,
        {
          trigger_list_id:
  ...
  const runtimeEvents = useMemo<WorkflowRuntimeEvent[]>(() => {
    ...
    return logs.map((entry) => {
      ...
      const nodeIdFromMetadata =
        ...
        metadata.node_id.trim().length > 0
          ? metadata.node_id
          : undefined;
    ...
    >
      {initializing ? (
        ...
        <div className="space-y-4">
          <Card className="overflow-hidden border-[var(--shell-border)] bg-[var(--shell-surface)]">
            <CardContent className="space-y-4 p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <h1 className="flex items-center gap-2 text-2xl font-semibold text-[var(--shell-ink)]">
                    ...
                    <Badge variant="secondary">
                      {billingCredits === null
                        ? "Credits unavailable"
                        : `${billingCredits.toLocaleString()} credits`}
                    </Badge>
                  </div>
                  <p className="max-w-3xl text-sm text-[var(--shell-muted)]">
                    Build, review, and launch automations from one workspace.
                    Templates stay available without pushing the editor below
                    the fold.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Button
                    variant="outline"
                    onClick={() => action("refresh", loadData)}
                    disabled={!!busy}
                  >
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4">
                  ...
                  <p className="text-xs text-emerald-700/80">
                    Contacts currently moving through flows
                  </p>
                </div>
                <div className="rounded-2xl border border-blue-200 bg-blue-50/80 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-blue-700/80">
                    Due now
                  </p>
                  ...
                  <p className="text-xs text-blue-700/80">
                    Pending actions ready for processing
                  </p>
                </div>
                <div className="rounded-2xl border border-[var(--shell-border)] bg-white/80 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--shell-muted)]">
                  ...
                  <p className="mt-2 text-2xl font-semibold text-[var(--shell-ink)]">
                    {supportedTemplates.length}
                  </p>
                  <p className="text-xs text-[var(--shell-muted)]">
                    Reusable starting points for new journeys
                  </p>
                </div>
              </div>
              ...
              <Tabs
                value={workspaceView}
                onValueChange={(value) =>
                  setWorkspaceView(value as "builder" | "templates")
                }
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <TabsList className="grid w-full max-w-[340px] grid-cols-2 bg-slate-100">
                    <TabsTrigger value="builder">Builder workspace</TabsTrigger>
                  ...
                  <p className="text-sm text-[var(--shell-muted)]">
                    {workspaceView === "builder"
                      ? "Select a workflow, adjust the essentials, then stay in the canvas."
                      : "Browse templates separately, preview the flow, and copy one into the active workflow when ready."}
                  </p>
                </div>
                  ...
                  >
                                  ...
                                  <div className="mt-2 flex flex-wrap items-center gap-2">
                                    <Badge
                                      className={getApprovalBadgeClass(
                                        getWorkflowApprovalStatus(workflow),
                                      )}
                                    >
                                      {approvalLabel(
                                        getWorkflowApprovalStatus(workflow),
                    ...
                    {!editor || !initialGraph ? (
                      ...
                      <div className="space-y-4">
                        {workspaceMode ? (
                          <Card className="border-[var(--shell-border)] bg-[var(--shell-surface)]">
                            <CardContent className="grid gap-3 p-3 lg:grid-cols-[minmax(0,320px)_1fr] lg:items-center">
                              <div className="space-y-2">
                          ...
                          <CardContent className="space-y-3 p-4">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <div className="space-y-2">
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <span className="text-xs text-[var(--shell-muted)]">
                                    Last run: {formatDate(editor.last_run_at)}
                                  </span>
                                </div>
                                <div>
                                  <p className="text-lg font-semibold text-[var(--shell-ink)]">
                                    {editor.name || "Untitled automation"}
                                  </p>
                                  <p className="text-sm text-[var(--shell-muted)]">
                                    Keep setup lightweight here. Detailed
                                    editing stays in the canvas and node
                                    inspector below.
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                <Button
                                  ...
                                  onClick={() =>
                                    action("duplicate", async () => {
                                      if (!user) return;
                                      const dup =
                                        await duplicateAutomationWorkflow(
                                          user.id,
                                          editor,
                                        );
                                ...
                                {editor.status !== "live" ? (
                                  canSubmitAutomationForApproval ||
                                ...
                                <Button
                                  variant="outline"
                                  onClick={() => {
                                    setEditorPanel("advanced");
                                    setShowWorkflowSettings(true);
                                  }}
                                  disabled={!!busy}
                                >
                                  {showWorkflowSettings ||
                                  editorPanel === "advanced" ? (
                            ...
                            selectedApprovalStatus !== "approved" ? (
                              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                                Workflow activation is blocked until this
                            ...
                            <Tabs
                              ...
                              onValueChange={(value) => {
                                const nextValue = value as
                                  | "essentials"
                                  | "advanced";
                                setEditorPanel(nextValue);
                                setShowWorkflowSettings(
                                  nextValue === "advanced",
                                );
                              }}
                            >
                              <TabsList className="grid w-full max-w-[300px] grid-cols-2 bg-slate-100">
                                ...
                                <div className="grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_repeat(3,minmax(0,1fr))]">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <Select
                                      ...
                                      onValueChange={(value) =>
                                        setTriggerType(
                                          value as AutomationWorkflow["trigger_type"],
                                        )
                                      }
                                    >
                                      <SelectTrigger>
                                      ...
                                      <SelectContent>
                                        ...
                                        editor.trigger_type ===
                                          ...
                                          <SelectItem
                                            value="custom_event"
                                            disabled={!supportsApiWebhooks}
                                          >
                                            {supportsApiWebhooks
                                              ? "Webhook event"
                                              : "Webhook event (Upgrade required)"}
                                          </SelectItem>
                                        ) : null}
                                      </SelectContent>
                                    </Select>
                                  ...
                                  <div className="space-y-2">
                                          ...
                                          prev
                                            ? {
                                                ...
                                                trigger_filters: (() => {
                                                  const nextFilters = {
                                                    ...toObject(
                                                      prev.trigger_filters,
                                                    ),
                                                  };
                                                  const currentSegmentId =
                                                    String(
                                                      nextFilters.segment_id ||
                                                        "",
                                                    ).trim();
                                                  if (!currentSegmentId)
                                                    return nextFilters;
                                                  ...
                                                  const matchedSegment =
                                                    dependencies.contactSegments.find(
                                                      (segment) =>
                                                        segment.id ===
                                                        currentSegmentId,
                                                    );
                                    ...
                                    >
                                      <SelectTrigger
                                        disabled={
                                          editor.trigger_type !== "list_joined"
                                        }
                                      >
                                        <SelectValue />
                                      </SelectTrigger>
                                    ...
                                    >
                                      <SelectTrigger
                                        disabled={
                                          editor.trigger_type !==
                                            "list_joined" ||
                                          availableTriggerSegments.length === 0
                                        }
                                      >
                                        <SelectValue />
                                      </SelectTrigger>
                              ...
                              >
                                <div className="grid gap-3 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                                  ...
                                  <div className="rounded-2xl border border-[var(--shell-border)] bg-slate-50/70 p-4">
                                    <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--shell-muted)]">
                                      Automation guidance
                                    </p>
                                    <div className="mt-3 space-y-2 text-sm text-[var(--shell-muted)]">
                                      <p>
                                        Use a short, outcome-based name so the
                                        workflow list stays easy to scan.
                                      </p>
                                      <p>
                                        Keep triggers specific. Narrow list and
                                        segment choices reduce accidental
                                        enrollments.
                                      </p>
                                      <p>
                                        Use the canvas inspector for step-level
                                        detail instead of overloading the
                                        description.
                                      </p>
                                    </div>
                                  </div>
                            ...
                            {!supportsApiWebhooks ? (
                              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                                API and webhook-triggered automations are
                              ...
                              <div className="space-y-3 rounded-xl border border-cyan-200 bg-cyan-50/60 p-3">
                                ...
                                <div className="grid gap-3 lg:grid-cols-2">
                                  <div className="space-y-2">
                                    ...
                                    <Input
                                      ...
                                      onChange={(event) =>
                                        setWebhookEventName(event.target.value)
                                      }
                                      placeholder="contact_created"
                                    />
                                    <p className="text-xs text-cyan-700/80">
                                      If set, only matching webhook events will
                                      enroll contacts.
                                    </p>
                                  </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <p className="text-xs text-cyan-700/80">
                                    ...
                                    <code>email</code>. You can pass
                                    <code>name</code> and <code>data</code>{" "}
                                    fields for personalization and conditions.
                                  </p>
                                </div>
                                    ...
                                    {webhookTestState.status !== "idle" ? (
                                      ...
                                      >
                                        ...
                                        {webhookTestState.testedAt
                                          ? ` (${formatDate(webhookTestState.testedAt)})`
                                          : ""}
                                      </p>
                                    ) : (
                                      <p className="text-xs text-cyan-700/80">
                                        Sends a live test payload to verify the
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-[var(--shell-muted)]">
                            Start from proven flows, preview the structure, then
                            copy one into the active workflow.
                          </p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <Badge variant="secondary">
                            {supportedTemplates.length} available
                          </Badge>
                          {editor ? (
                            <Badge variant="secondary">
                              Active target: {editor.name}
                            </Badge>
                          ) : null}
                        </div>
                      </div>
                      ...
                      {supportedTemplates.length === 0 ? (
                        ...
                        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
                          {supportedTemplates.map((template) => {
                            const previewGraph =
                              getTemplatePreviewGraph(template);
                            const previewSteps = previewGraph.nodes.filter(
                              (node) =>
                                node.kind !== "trigger" && node.kind !== "exit",
                            ).length;
                            return (
                              <div
                                key={template.id}
                                className="rounded-2xl border border-[var(--shell-border)] bg-white/90 p-4 shadow-sm"
                              >
                                ...
                                <p className="mt-3 line-clamp-3 text-sm text-[var(--shell-muted)]">
                ...
                <>
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Badge variant="outline">
                        {triggerTypeLabel(selectedTemplate.trigger_type)}
                      </Badge>
                      <Badge variant="outline">
                        {
                    ...
                    {!selectedTemplate.runner_compatible ? (
                      <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                        This template contains blocks that are not fully
                        runner-compatible yet. It can be edited now, but publish
                        may stay blocked until those blocks are removed.
                      </div>
                    ) : null}
