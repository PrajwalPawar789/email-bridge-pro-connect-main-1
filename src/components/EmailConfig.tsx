            ...
            {configs.length === 0 ? (
              ...
              <div className="space-y-3">
                {configs.map((config) => {
                  ...
                  const isMissingName = !config.sender_name;
                  return (
                    <div key={config.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
                      <div className="flex items-start gap-3">
                        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-100 text-sm font-semibold text-emerald-700">
                          {(config.sender_name || config.smtp_username || 'E').charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="text-sm font-semibold text-slate-900">{config.sender_name || 'Sender name missing'}</p>
                            {isMissingName && (
                          ...
                          <p className="text-xs text-slate-400">{providerLabel} • {config.smtp_host}:{config.smtp_port}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" className="rounded-full" onClick={() => openEditForm(config)}>
        ...
        <div className="space-y-4">
          <Card className="border border-slate-200 bg-slate-50/80">
            <CardContent className="space-y-2 p-6">
              ...
              <p className="text-sm text-slate-600">{helpBody}</p>
              {!showForm && (
                <Button onClick={openCreateForm} className="mt-3 rounded-full">
                  Add configuration
                </Button>
              )}
            </CardContent>
