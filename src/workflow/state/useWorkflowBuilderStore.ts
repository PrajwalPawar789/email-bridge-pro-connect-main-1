...
interface WorkflowBuilderState {
  ...
  setGraph: (
    graph: WorkflowGraph,
    options?: { resetHistory?: boolean; markDirty?: boolean },
  ) => void;
  replaceGraph: (
    graph: WorkflowGraph,
    options?: {
  ...
  updateGraphMeta: (
    patch: Partial<Pick<WorkflowGraph, "name" | "status" | "version">>,
  ) => void;
...
const withHistory = (
  set: (
    fn: (state: WorkflowBuilderState) => Partial<WorkflowBuilderState>,
  ) => void,
  ...
  (set, get) => ({
    ...
    setRuntimeEvents: (events) =>
      set((state) => {
        ...
        if (
          ...
          state.runtimeEvents.every(
            (event, index) => event.id === events[index]?.id,
          )
        ) {
          return state;
        }
    ...
    disconnectNode: (nodeId) =>
      withHistory(set, (state) => ({
        ...
        selectedNodeIds: state.graph.nodes.some((node) => node.id === nodeId)
          ? [nodeId]
          : [],
        selectedEdgeIds: [],
      })),
        ...
        return {
          graph: {
            ...
            nodes: state.graph.nodes.filter(
              (node) => !selectedNodeSet.has(node.id),
            ),
            edges: state.graph.edges.filter(
              (edge) =>
    ...
    pasteClipboard: () => {
      ...
      withHistory(set, (prev) => {
        ...
        const pastedNodes: WorkflowNode[] = state.clipboard!.nodes.map(
          (node) => {
        ...
        const pastedEdges: WorkflowEdge[] = state
